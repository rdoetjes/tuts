...
// Setup PulseAudio for playback using the parameters defined in the header.
bool setup_pulseaudio(pa_simple*& handle) {
    ...
    int error;
    handle = pa_simple_new(NULL, "bluebox", PA_STREAM_PLAYBACK, NULL, "playback", &ss, NULL, NULL, &error);
    if (!handle) {
        std::cerr << "setup_pulseaudio: pa_simple_new() failed: " << pa_strerror(error) << "\n";
        return false;
    }
...
// Write frames to PulseAudio. Returns true on success.
bool write_frames(pa_simple* handle, const int16_t* data, size_t frame_count) {
    if (handle == nullptr) return false;

    int error;
    if (pa_simple_write(handle, data, frame_count * sizeof(int16_t), &error) < 0) {
        std::cerr << "write_frames: pa_simple_write() failed: " << pa_strerror(error) << "\n";
        return false;
    }
...
static void play_h_step(pa_simple* handle, const parsing::SequenceStep& step, size_t index, int serial_fd, const std::vector<int16_t>& silence_chunk) {
    if (serial_fd < 0) {
...
static void play_tone_step(pa_simple* handle, const parsing::SequenceStep& step, size_t index, const std::vector<int16_t>& silence_chunk) {
    int f1 = 0, f2 = 0;
    bool valid = false;
