...
@ Dictionary Linker Management
.set _link, 0

@ defcode "name", len, flags
...
@ Forth Assembly Macros for pure syntax
...
@ Calling Macros for words with legal names
...
4:  ldr r0, =CMSDK_UART0_BASE; movs r1, #3; str r1, [r0, #CMSDK_UART_CTRL_OFFSET]
    ...
    @ Initialize dictionary pointer to the last defined builtin word
    ldr r0, =_latest
...
v1: ldrb r6, [r1, r7]; strb r6, [r3, r7]; adds r7, #1; cmp r7, r0; blt v1
    add r3, r0; adds r3, #3; lsrs r3, #2; lsls r3, #2
    ldr r6, =0x3d04; strh r6, [r3]      @ adds r5, #-4
    ldr r6, =0x602c; strh r6, [r3, #2]  @ str r4, [r5]
    ldr r6, =0xa400; strh r6, [r3, #4]  @ adr r4, data
    ldr r6, =0x4770; strh r6, [r3, #6]  @ bx lr
    movs r6, #0; str r6, [r3, #8]       @ data cell
    adds r3, #12
    str r3, [r2]; pop {pc}
